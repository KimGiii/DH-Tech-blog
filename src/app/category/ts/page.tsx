...
async function getPosts() {
    const res = await fetch("http://localhost:3000/api/post?category=TYPESCRIPT", {
        next: { revalidate: 60 },
    });
    if (!res.ok) {
...
export default async function JavaScriptCategoryPage() {
    ...
    return (
        <main className="max-w-4xl mx-auto px-4 py-8">
            <h1 className="text-2xl font-bold mb-6">📂 TypeScript 게시글</h1>
            <ul className="space-y-6">
                {posts.map((post: any) => (
                    <li key={post.id} className="border-b pb-4">
                        <Link href={`/category/ts/${post.id}`}>
